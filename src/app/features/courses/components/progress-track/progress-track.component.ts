...
import { Component, Input, AfterViewInit, OnDestroy, ElementRef, ViewChild, OnChanges, SimpleChanges, ViewEncapsulation } from '@angular/core';
...
export class ProgressTrackComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  @Input() flagPositions: number[] = [10,30,40,77,80];
  @Input() targetPercentage: number = 80;
  @Input() currentPercentage: number = 60;
  @Input() animationDuration: number = 3500;
  @Input() alreadyCompleted: boolean = false;
  ...
  ngAfterViewInit() {
    ...
    const lastProgress = localStorage.getItem("last-progress")
    if (lastProgress && this.currentPercentage == parseInt(lastProgress)){
      this.alreadyCompleted = true
    } 

    // save the new progress in local storage
    
    localStorage.setItem("last-progress", this.targetPercentage.toString())
  }
  ...
  private initializeComponent() {
    ...
    try {
      ...
      if (this.flagPositions.length === 0) {
        this.flagPositions = Array.from({ length: this.months }, (_, index) => (index + 1) * 100 / this.months);
        console.log(this.flagPositions)
      }

      this.flags = this.flagPositions.map((_, index) => {
      ...
      const frameRate = 60;
      const totalFrames = (this.targetPercentage/15) * frameRate;
      ...
      // Position flags
      this.flagPositions.forEach((pos, index) => {
      ...
      const animate = () => {
        if (currentOffset > targetLength) {
          ...
          this.car.style.translate = `${point.x - 30}px ${point.y - 40}px`;
          // this.car.style.outline = '1px solid red';
          this.car.style.transform = `rotate(${0}deg)`;
          if (progress > 0.33 && progress < 0.66){
            
            // rotate the car around the y-axis
            this.car.style.transform= `rotateY(180deg)`;  
            this.car.style.translate = `${point.x+30}px ${point.y - 40}px`;
          }
          ...
          // this.car.setAttribute('transform', `translate(${point.x},${point.y}) scale(0.1)`);
          ...
          // you have completed ${this.targetPercentage}% of the course
          this.toast.show(`You have completed ${this.targetPercentage}% of the course`);
          this.car.classList.add('done')
        }
      };
      ...
      if (this.alreadyCompleted){
        // Directly show the car at the end of the path
        const point = path.getPointAtLength(progressPathLength);
        this.car.style.translate = `${point.x - 30}px ${point.y - 40}px`;
        this.car.style.transform = `rotate(${0}deg)`;
  ...
  private createCar(): SVGGElement {
    ...
    car.innerHTML = `<path id="car-front-window" style="fill:#C5E2E2;" d="M335.665,129.056l33.596,72.257H208.373V118.25h110.265   C323.998,118.25,333.388,124.208,335.665,129.056z"/>
		...
		<path id="car-headlight" style="fill:#EEC278;" d="M459.504,219.223c24.38,8.127,44.326,35.796,44.326,61.494c-25.807,0-46.723-20.916-46.723-46.723   C457.107,228.831,457.946,223.874,459.504,219.223z"/>
		<g id="car-body">
